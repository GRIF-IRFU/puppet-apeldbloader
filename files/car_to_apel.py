...
if not arcdir:
    print "ARC " + ARC_archives_option + " option in " + ARC_archives_section + " section of "+ARC_config+" does not seem to contain an archiving dir suboption : cannot parse that dir for apel. exiting."
    exit(2)

print "Processing ARC archives in " + arcdir + "..."
...
for root, dirs, files in os.walk(arcdir,topdown=False):
     for name in dirs:
         ...
         if re.compile('[0-9]{8,8}').match(name) and not os.listdir(fname): #to check wither the dir is empty and of the form 01234567 (the dirq tmp dirs)
             print "removing emtpty dir " + fname
             os.removedirs(fname)
  ...
  for filename in os.listdir('.'):
      if filename.startswith("usagerecordCAR"):
          ...
          if filename not in tarmembers:
            #print 'adding ' + filename + ' to ' + tarfileN
            ...
            print "WARNING : file " + filename + " already in archive " + tarfileN + " : something might be going wrong. Moving it in archives/failed"
